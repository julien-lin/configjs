...
import { promptNextjsSetup } from '../prompts/nextjs-setup.js'
import { createNextjsProject } from '../utils/nextjs-installer.js'
 ...
 */
export async function installNextjs(options: CLIOptions): Promise<void> {
  try {
    ...
    // Vérifier que le framework détecté est bien Next.js
    if (ctx.framework !== 'nextjs') {
      console.log()
      console.log(
        pc.yellow(
          `⚠️  Framework détecté: ${ctx.framework}. Cette commande est destinée aux projets Next.js.`
        )
      )
      console.log(
        pc.gray(
