...
`
  ...
  it('should support --yes flag to skip prompts', async () => {
    ...
    await fs.writeFile(
      configPath,
      JSON.stringify({
        plugins: ['react-router-dom', 'zustand', 'tailwindcss', 'eslint'],
      })
    )
  ...
  it('should support --dry-run to preview changes', async () => {
    ...
    expect(JSON.stringify(projectBefore)).toBe(JSON.stringify(projectAfter))
  })
  ...
  it('should support --no-install flag', async () => {
    ...
    const depsCountBefore = Object.keys(pkgBefore['dependencies'] || {}).length

    // Après exécution avec --no-install
    const pkgAfter = await readPackageJson(projectPath)
    const depsCountAfter = Object.keys(pkgAfter['dependencies'] || {}).length

    // Le nombre de dépendances ne devrait pas augmenter
    expect(depsCountAfter).toBeLessThanOrEqual(depsCountBefore + 1) // react et react-dom déjà là
  })
  ...
  it('should filter plugins by category with "list" command', async () => {
    ...
    const routingPlugins = ['react-router-dom', 'tanstack-router']
  ...
  it('should show plugin details in list', async () => {
    ...
    const expectedFields = ['Name', 'Version', 'Category', 'Compatible with']
  ...
  it('should format output without colors in --silent mode', async () => {
    ...
    const configPath = join(projectPath, '.confjs.json')
    await fs.writeFile(configPath, JSON.stringify({ plugins: ['zustand'] }))
