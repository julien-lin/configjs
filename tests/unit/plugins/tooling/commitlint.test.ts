...
import { commitlintPlugin } from '../../../../src/plugins/tooling/commitlint.js'
...
describe('Commitlint Plugin', () => {
  ...
  describe('detect', () => {
    it('should detect commitlint in devDependencies', () => {
      const ctx = {
        ...mockContext,
        devDependencies: { '@commitlint/cli': '^20.0.0' },
      }
      expect(commitlintPlugin.detect?.(ctx)).toBe(true)
    })
  })

  describe('install', () => {
    it('should install commitlint packages', async () => {
      const result = await commitlintPlugin.install(mockContext)
      expect(result.success).toBe(true)
      expect(result.packages.devDependencies).toContain('@commitlint/cli')
    })
  })

  describe('configure', () => {
    it('should create config and update package.json', async () => {
      const result = await commitlintPlugin.configure(mockContext)
      expect(result.success).toBe(true)
  ...
  describe('rollback', () => {
    it('should restore backups', async () => {
      await commitlintPlugin.rollback?.(mockContext)
