...
import { unpluginAutoImportPlugin } from '../../../../src/plugins/tooling/unplugin-auto-import.js'
...
describe('unplugin-auto-import Plugin', () => {
  ...
  describe('detect', () => {
    it('should detect unplugin-auto-import in devDependencies', () => {
      const ctx = {
        ...mockContext,
        devDependencies: { 'unplugin-auto-import': '^20.0.0' },
      }
      expect(unpluginAutoImportPlugin.detect?.(ctx)).toBe(true)
    })
  })

  describe('install', () => {
    it('should install unplugin-auto-import', async () => {
      const result = await unpluginAutoImportPlugin.install(mockContext)
      expect(result.success).toBe(true)
      expect(result.packages.devDependencies).toContain('unplugin-auto-import')
    })
  })

  describe('configure', () => {
    it('should update vite config', async () => {
      const result = await unpluginAutoImportPlugin.configure(mockContext)
      expect(result.success).toBe(true)
  ...
  describe('rollback', () => {
    it('should restore backups', async () => {
      await unpluginAutoImportPlugin.rollback?.(mockContext)
